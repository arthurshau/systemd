...
static int query_bpf_progs_and_maps_reply(
        ...
        if (streq(json_variant_string(m), "MAP")) {
                ...
                log_info(" reply: map_name = %s, map_type = %lu, memlock = %s", p.name, p.type, p.memlock);
        } else {
                static const JsonDispatch dispatch_table[] = {
                ...
                log_info(" reply: prog_name = %s, prog_type = %lu, memlock = %s, prog_tag = %s, prog_map_names = %s, run_time_ns = %lu, run_cnt = %lu", p.name, p.type, p.memlock, p.tag, p.map_names, p.run_time_ns, p.run_cnt);
        }
...
static int verb_asdf(int argc, char *argv[], void *userdata) {
        _cleanup_(varlink_unrefp) Varlink *vl = NULL;
        JsonVariant *v = NULL;
        const char *err = NULL;
        ...
        char method[] = "io.systemd.BpfProgsAndMaps.GetBpfProgsAndMaps";
        r = varlink_collect(vl, method, NULL, &v, &err, &flags);
        if (r < 0)
                return log_debug_errno(r, "Failed to call varlink method: %m");

        json_variant_dump(v, JSON_FORMAT_PRETTY, NULL, NULL);
        log_info("error: %s, flags: %u", err, flags);
        return r;
}
...
static int run(int argc, char *argv[]) {
        static const Verb verbs[] = {
                ...
                { "asdf", VERB_ANY, VERB_ANY, 0, verb_asdf },
                {}
        };
